void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if(dot(uv,uv) < (0.01*sin(iGlobalTime*(2.))+0.04)) {
        fragColor = vec4(0.42, 0.2, 0.4,1.0);
        if((uv.x * uv.x) +  (uv.y * uv.y) < (0.012*sin(iGlobalTime*(4.))+0.016)) {
        	fragColor = vec4(0.3, 0.3, 0.4,1.0);
    	} 
        if((uv.x * uv.x) +  (uv.y * uv.y) < (0.008*sin(iGlobalTime*(4.+timeInc*1.)+mult*1.)+0.011)) {
        	fragColor = vec4(0.3, 0.7, 0.4,1.0);
    	} 
        if((uv.x * uv.x) +  (uv.y * uv.y) < (0.004*sin(iGlobalTime*(4.+timeInc*2.)+mult*2.)+0.006)) {
        	fragColor = vec4(0.1, 0.7, 0.7,1.0);
    	} 
        if((uv.x * uv.x) +  (uv.y * uv.y) < (0.0005*sin(iGlobalTime*(4.+timeInc*3.)+mult*3.)+0.001)) {
