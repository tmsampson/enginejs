...
vec2 map( vec3 p )
{
    ...
    // ground
    vec3 s = p;
    s.y -= 0.1*sin( 0.25*s.z + 0.5*sin(0.25*s.x) );    
    s.y += 0.1*cos( 0.25*s.z + 0.5*cos(0.25*s.x) );        
    float d2 = s.y - 0.17;
    if( d2<2.0 )
    {
    d2 += 0.06*texture2D( iChannel2, 0.15*s.xz ).x;
    d2 -= 1.5*pow(texture2D( iChannel3, 0.0 + 0.01*s.xz, -8.0 ).x,0.35) - 0.8;
    }
    d2 *= 0.8;
    ...
    if( dot(pid,pid)>0.5 ) 	p.xz += 1.0*(-1.0+2.0*hash2( vec2(313.1*pid.x + 171.4*pid.y,331.8*pid.x + 153.4*pid.y) ));
#endif
    ...
    if( dot(pid,pid)>0.5 )
    {
        ...
        d3 -= 0.1*sqrt(texture2D( iChannel2, 0.1*s.zy ).x);
        //d3 *= 0.5;
...
vec4 texcube( sampler2D sam, in vec3 p, in vec3 n, in float k )
{
	...
	vec4 x = texture2D( sam, p.yz, b );
	vec4 y = texture2D( sam, p.zx, b );
	vec4 z = texture2D( sam, p.xy, b );
...
vec3 shade( in vec3 ro, in vec3 rd, in float t, in float m )
{
    ...
    if( m<0.5 )
    {
        ...
        mateD *= 0.05 + 2.0*pow(1.0-texcube( iChannel3, pos*0.25, nor, 1.0 ).xyz, vec3(6.0));
        ...
        vec3 onn = 0.1*(-1.0+2.0*texcube( iChannel1, pos*1.0, nor, 1.0 ).xyz );
	...
	else if( m<1.5 )
    {
        ...
        float ll = smoothstep( 0.2, 0.3, texture2D(iChannel3,pos.xz*.1).x );
	...
	else //if( m<3.5 )
    {
        ...
        mateD *= 0.2 + 0.8*texcube( iChannel2, pos*0.07, nor, 1.0 ).x;
        mateD *= .14;
    }
