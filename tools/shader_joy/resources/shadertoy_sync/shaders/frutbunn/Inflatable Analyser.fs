...
void colorize(in float d, in vec3 c, inout float cd, inout vec3 cc) {
    const float md = .25;
    const float b = 1.25;
    
    float nc = smoothstep(d-b, d+b, cd);
    float nd = smoothstep(d-md, d+md, cd);
...
float rayMarch(in vec3 origin, in vec3 ray, out vec3 col) {
    ...
    float cd = 1000.;
    colorize(o[2], vec3(.4*scol[0], .2*scol[6], .6*scol[4]), cd, col );
	colorize(o[3], vec3(0., 0., 0.), cd, col );
    colorize(o[0], vec3(.6*scol[1], 1.*scol[5], 0.), cd, col );
    colorize(o[1], vec3(.5*scol[2], 0., .3*scol[3]), cd, col );
    col = clamp(col, 0., 1.);
