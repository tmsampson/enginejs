...
vec4 FreqAnalysis() {
    ...
    for(float i = 0.0; i < 0.25; i += 0.002) {
   		float c = texture2D(iChannel0, vec2(i, 0.0)).x;
        sy.x = max(c, sy.x);
        
    }
    
    for(float i = 0.25; i < 0.5; i += 0.002) {
   		float c = texture2D(iChannel0, vec2(i, 0.0)).x;
        sy.y = max(c, sy.y);
        
    }
    
    for(float i = 0.5; i < 0.75; i += 0.002) {
   		float c = texture2D(iChannel0, vec2(i, 0.0)).x;
        sy.z = max(c, sy.z);
        
    }
    
    for(float i = 0.75; i <= 1.0; i += 0.002) {
   		float c = texture2D(iChannel0, vec2(i, 0.0)).x;
        sy.w = max(c, sy.w);
        
    }
    
    //sy = sy / max(max(sy.x, sy.y), max(sy.z, sy.w));
#else
...
float noise(in vec3 p) {
    ...
    return clamp((smoothstep(0.0, 1.0, mix(0.1, 0.2, v*10.0-f)) + f)* 0.1, 0.0, 1.0);
...
bool RayMarchPerturbedSphere(in vec3 ro, in vec3 rd, in vec3 c, in float r, in float br, 
    ...
    float dist = res ? length(p0 - bp0) : length(bp0 - bp1);

    const float sc = 128.0;
    float s = dist / sc;
        
    for (float d = 0.0; d < sc; ++d) {
    	vec3 pn = (bp0 + d*s*rd) - c;
...
float ShadowFactor(in vec3 sd, in vec3 ld, in vec3 c, in float r, in float br) {
    ...
    for (float d = 0.0; d < sc; ++d) {
        ...
        vec3 sd1 = normalize(pn) * r;
        float h = length(pn) - r + s;
        
        float h0 = noise(sd1);
        if (h0 > h+0.01) return 0.0;
...
vec3 GetColor(vec3 sd) {
    float n = noise(sd);
    /*
    return ColorPalette(n, vec3(0.5, 0.5, 0.5), vec3(0.5, 0.5, 0.5),
                           vec3(1.0, 1.0, 0.5), vec3(0.4, 0.3, 0.3));*/
...
vec3 CameraRay(vec2 uv, float n) {
    float a = 1.0/max(iResolution.x, iResolution.y);
    
    
    return normalize(vec3(uv*a - iResolution.xy*a*0.5, n));
}
...
{   
    ...
    if (hit) {
        ...
        color = GetColor(sd);
        vec3 diff = color * max(dot(-l, n), 0.0 ) * 0.95 * sf;
        vec3 amb = color * 0.05;
