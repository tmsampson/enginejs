...
{
    #if 1
        float CP0X = CubicLagrange(CP00, CP01, CP02, CP03, P.x);
        float CP1X = CubicLagrange(CP10, CP11, CP12, CP13, P.x);
        float CP2X = CubicLagrange(CP20, CP21, CP22, CP23, P.x);
        float CP3X = CubicLagrange(CP30, CP31, CP32, CP33, P.x);

        return CubicLagrange(CP0X, CP1X, CP2X, CP3X, P.y);
    #else
        float CPX0 = CubicLagrange(CP00, CP10, CP20, CP30, P.y);
        float CPX1 = CubicLagrange(CP01, CP11, CP21, CP31, P.y);
        float CPX2 = CubicLagrange(CP02, CP12, CP22, CP32, P.y);
        float CPX3 = CubicLagrange(CP03, CP13, CP23, CP33, P.y);

        return CubicLagrange(CPX0, CPX1, CPX2, CPX3, P.x);
    #endif
}
