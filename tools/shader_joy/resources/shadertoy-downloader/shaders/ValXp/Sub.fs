...
vec2 map(vec3 position)
{
    ...
    position.y -= 1.0;
    float value = texture2D( iChannel0, vec2( 0.01, 0.25 ) ).x / 2.0;
    position.z += value;
    ...
    return matMin(vec2(smin(centerSphere, cone, 0.2), 1.0), vec2(box, 2.0));
}
...
vec4 shade(vec4 intersection)
{
    ...
    if (intersection.a == 2.0) // box
    {
        ...
        diffuse = texture2D(iChannel1, vec2(intersection.x / 5.0, intersection.y / 5.0)).xyz * 0.6;
