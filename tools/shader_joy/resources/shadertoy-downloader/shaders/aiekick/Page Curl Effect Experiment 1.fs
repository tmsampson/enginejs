...
/////////////////////////////////////////////////////////
// Flowerfrom iq shader https://www.shadertoy.com/view/4dX3Rn
vec3 FlowerEffect(vec2 v) 
{
   	vec2 p = v;
...
/////////////////////////////////////////////////////////
// Strawberry milk simplified version from my 2TC 15 Shader 
// https://www.shadertoy.com/view/MlfGWN
vec3 StrawBerryMilkEffect(vec2 v)
{
    vec2 s = v;
    
    float 
        z = 25.,
        r = iResolution.x/iResolution.y*z,
        p, c;
    ...
    s += p * cos( p - vec2(9.4,8.6) * iGlobalTime *.6 ); 
    
    c += 2. / dot(s, s); 

    c = smoothstep(c -2., c +1.2, 1.);

   	return vec3(c, -2./c + c*3., -4./c + c*5.);
}
...
vec4 DrawFront(vec2 uv) // strawberrymilk
{
    ...
    return StrawBerryMilkEffect(uv).rgbb;
}
