...
vec3 texCol( in vec3 p){
    ...
    //return texture2D(iChannel0, u4*4.).xyz*c; 
    
    // etc.
}

float texShade(vec3 p){
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
	...
	*/
    ...
    //if(sDist>1e-8){
    	...
    	vec3 sn =  normalize(-sign(sp)*vec3(abs(rd.x)>abs(rd.y) ? vec2(1., 0.) : vec2(0., 1.), 0.)); // Square normal.
    	
    
        // Coloring the surface.
        vec3 objCol = texCol(sp);
        ...
        vec3 gr = (vec3(texShade(sp-eps.xyy), texShade(sp-eps.yxy), texShade(sp-eps.yyx))-c)/eps.x;
