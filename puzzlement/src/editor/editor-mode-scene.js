Editor.Mode_Floor = function()
{
	...
	this.Update = function()
	{
		...
		this.SelectedTile = null; // Reset
		this.SelectedTileHitPos = Engine.Intersect.RayPlane(this.SelectedTileHitPos, Core.Camera.position, Core.Camera.forward, [0, 1, 0], 0);
		if(this.SelectedTileHitPos != null)
		{
			this.SelectedTile = Core.WorldToCell(this.SelectedTileHitPos);
		}
	...
	{
		var tile_selected = this.SelectedTile != null;
		if(!tile_selected)
		{
			Editor.SubText.Set("");
			return;
		...
		// Update debug text
		Editor.SubText.Set(this.SelectedTile);

		// Cache state
		var depth_was_enabled = Engine.Gfx.IsDepthTestEnabled();

		// Calculate selected tile
		var render_pos = Engine.Vec3.MultiplyScalar(this.SelectedTile, Core.Map.FloorTileSize);
		render_pos[1] += Constants.ZFightOffset; // Prevent z-fighting with ground

		// Colour selected tile
		var selected_tile_colour = Core.IsValidTile(this.SelectedTile)? Engine.Colour.Green : Engine.Colour.Red;
		var pulse_speed = 10;
		...
		this.SelectedTileHitPos[0] += cursor_size * 0.5; this.SelectedTileHitPos[2] += cursor_size * 0.5; // Centre cursor
		this.SelectedTileHitPos[1] += (Constants.ZFightOffset * 2.0); // Prevent z-fighting with selected tile
		Engine.Gfx.BindMaterial(Editor.CursorMaterial);
		mat4.translate(Core.ScratchMatrix, Engine.Math.IdentityMatrix, this.SelectedTileHitPos);
	...
	this.ToggleSelectedTile = function(direction)
	{
		...
		var cell_id = Core.GetCellId(this.SelectedTile);
